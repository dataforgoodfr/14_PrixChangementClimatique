            ...
            for row_idx, (score_name, raw) in enumerate(scores.items()):
                ...
                if overlays:
                    ...
                    for ov_idx, (ov_label, ov_gdf) in enumerate(overlays.items()):
                        ...
                        mask = ov_gdf.index()
                        ov_transformed = transformed.loc[mask]
...
def _(metropole_datas):
    metropole_datas['swi_x_rga'].quantile(0.99)
    return
...
def _(metropole_datas):
    metropole_datas[
        [
            ...
            'swi_04_d_abs_indice',
            'indicateur_rga_indice',
            'swi_x_rga_indice',
            'nb_total_arretes_sec_indice',
            'swi_04_d_abs',
            'indicateur_rga',
            'nb_total_arretes_sec'
        ]
...
def _(MinMaxScaler, col_selector_inondation, gdf, np):
    ...
    datas_inondation['score_inondation_brut'] = np.sqrt(w_indicateur_tri * datas_inondation['indicateur_tri_indice']**2 +
...
@app.cell
def _(datas_inondation):
    datas_inondation.loc[
        ~datas_inondation['code_insee'].astype(str).str.startswith('98'),
        [
            'code_insee',
            'score_inondation_brut',
            'score_inondation',
            'rr_50_d_abs_indice',
            'indicateur_tri_indice',
            'nb_total_arretes_ino_indice'
        ]
    ]

    datas_inondation[['score_inondation_brut']].min()
    return
...
@app.cell
def _(gdf_calc):
    gdf_calc[ ['code_insee', 'score_inondation_brut', 'score_inondation',  'score_inondation_net' 'rr_50_d_abs_indice', 'indicateur_tri_indice', 'nb_total_arretes_ino_indice' ]]
    return
...
def _(
    ...
    print(gdf_calc_assurance['score_assurance'].max())
    return pna, ppb
...
def _(gdf_calc_assurance):
    cols = [
        ...
        'part_prime_budget_standard',
        'indice_prime',
        'multiple_franchise_last_indice',
        'evolution_prime_assurance_clip',
        'part_arretes_non_reconnus_clip'
    ]

    gdf_calc_assurance \
        .sort_values('score_assurance', ascending=False) \
        .loc[:, cols] \
        .head(10)
    return
...
@app.cell
def _(gdf_calc_assurance):
    gdf_calc_assurance.loc[gdf_calc_assurance['code_insee'] == '45269', ['score_assurance','score_assurance_min_max','part_prime_budget_standard','indice_prime', 'part_prime_budget_standard', 'multiple_franchise_last_indice','evolution_prime_assurance_clip', 'part_arretes_non_reconnus_clip']]
    return
...
def _(gdf_calc_assurance, np, poids_prevention_dettes, ppb):
    ...
    print(gdf_calc_eco['score_eco'].max())
    print(gdf_calc_eco['depenses_per_pop_indice'].max())
    print(gdf_calc_eco['debt_indice'].max())
    print(ppb)
    print(ppd)
...
@app.cell
def _(gdf_calc_eco):
    gdf_calc_eco.loc[gdf_calc_eco['code_insee'] == '28263', ['debt_indice', 'depenses_per_pop_indice', 'score_eco', 'score_eco_minmax']]
    return
...
def _(gdf_calc_eco, poids_prevention_assurance, poids_prevention_eco):
    ...
    gdf_calc_eco['score_final_brut'] = (ppeco*gdf_calc_eco['score_eco_minmax']**2+ppa*gdf_calc_eco['score_assurance_min_max']**2+ppexpo*gdf_calc_eco['score_exposition']**2)**(1/2)
...
@app.cell
def _(gdf_calc_eco):
    gdf_calc_eco['score_final_brut'].min()
    return
...
def _(colonne_all_score, gdf_calc_eco):
    ...
    _(colonne_all_score, gdf_calc_eco)
    return


@app.cell
def _(gdf_calc_eco):
    gdf_calc_eco[['code_insee', 'score_final']]
    return
